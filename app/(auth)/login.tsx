...
const API_URL = 'http://192.168.88.4:3000/api/auth/sign-in/email'
    ...
    <View className='justify-end h-full'>
      {/* <ImageBackground source={bg1}> */}
        ...
        <View className='h-[80%] flex items-center justify-evenly gap-4 bg-white rounded-tl-3xl rounded-tr-3xl'>
            <Text className='font-bold text-3xl my-6'>Se connecter</Text>
            <View className='w-[80%] flex gap-4'>
              ...
              <TouchableOpacity disabled={mutation.isPending} className='w-full py-4 mt-6 border-white border-2 rounded-full items-center justify-center bg-blue-800' onPress={() => mutation.mutate()}>
                {mutation.isPending ? <ActivityIndicator size={20}/> : <Text className='text-xl text-fuchsia-100 font-semibold'>Se connecter</Text>}
              </TouchableOpacity>
            </View>
          <View className='flex flex-row gap-3'>
            <Text className='text-base text-gray-400'>J'ai pas encore un compte !</Text>
            <TouchableOpacity onPress={() => router.push('/(auth)/signup')}>
              <Text className='text-[#6f74ff]'>S'inscrire</Text>
            </TouchableOpacity>
          </View>
        </View>
      {/* </ImageBackground> */}
